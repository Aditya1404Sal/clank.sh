...
//!
//! NB: this branch resolves golem-rust from the **dev SDK path dep**, where `get_self_metadata` and
//! `fork` ARE crate-root functions (and fallible), and the old `bindings::golem::api::host` module is
//! private. `main` still targets the released 2.1.0, where the reverse holds — that is the one real
//! divergence between the two lines in this file, and why the calls below are crate-root.
//!
...
fn self_metadata_text() -> clank_core::golem::error::Result<String> {
    // On the dev SDK `get_self_metadata` lives at the crate root and is FALLIBLE;
    // `AgentMetadata.agent_id` is a struct (no Display) whose `.agent_id` is the instance name.
    // A failure here is a host-surface fault the caller cannot correct — `Remote` (exit 4), not
    // `Unreachable` (which would advertise it as retryable; this is not a network condition).
    let md = golem_rust::get_self_metadata().map_err(|e| {
        clank_core::golem::Error::Remote(format!("self metadata unavailable: {e:?}"))
    })?;
    Ok(format!(
        "agent-id: {}\nstatus: {:?}\ncomponent-revision: {}\n",
...
impl GolemCluster for GolemApiCluster {
    async fn agent_list(&self) -> clank_core::golem::error::Result<String> {
    ...
    async fn agent_oplog(
        ...
        agent_type: &str,
        _ctor: &[(String, String)],
    ) -> clank_core::golem::error::Result<String> {
        Err(clank_core::golem::Error::Unsupported(format!(
    ...
    async fn connect(&self, identity: &str) -> clank_core::golem::error::Result<String> {
        Err(clank_core::golem::Error::Unsupported(format!(
            "golem connect '{identity}' needs an agent-id + streaming inspection (not wired in v1)"
        )))
    }

    async fn self_oplog(&self) -> clank_core::golem::error::Result<String> {
    ...
    async fn rollback(&self) -> clank_core::golem::error::Result<String> {
        ...
        Err(clank_core::golem::Error::Unsupported(
            "golem rollback needs an explicit revert target (oplog index or last-N) — not wired in v1"
                .to_string(),
        ))
    }

    async fn fork(&self) -> clank_core::golem::error::Result<String> {
        ...
        match golem_rust::fork() {
            ...
            Err(e) => Err(clank_core::golem::Error::Remote(format!(
                "fork failed: {e:?}"
            ))),
        }
    }
