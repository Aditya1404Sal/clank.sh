...
/// The top-level word splitter, which now lives in the shell core (the session layer uses it to
/// decide what a line asks for, and the core must not reach into a family module for it). Re-exported
/// here so `ask`'s callers keep resolving `ask::dequote_words`.
pub use crate::helpshim::dequote_words;
