...
//! handful of names Brush would miss ([`CORE_INTERCEPTED`], plus whatever an installed plug-in
//! reports as its own intercepted names) and otherwise returns `None` so
...
/// The commands the shell core intercepts before Brush dispatch — precisely the names in the
/// registry that are **not** registered as Brush builtins, so Brush's `type`/`--help` never see
/// them. `type` and `--help` handling own exactly this set plus the installed plug-in's own
/// intercepted names; everything else defers to Brush.
pub const CORE_INTERCEPTED: &[&str] = &["prompt-user", "curl", "wget", "context", "kill"];

/// Whether `name` is an intercepted command (one Brush can't resolve): a core one, or one of
/// `intercepted` — the installed plug-in's own list.
#[must_use]
pub fn is_intercepted(name: &str, intercepted: &[&str]) -> bool {
    CORE_INTERCEPTED.contains(&name) || intercepted.contains(&name)
}
...
pub fn dispatch(
    ...
    if !inv.names.iter().all(|n| is_intercepted(n, intercepted)) {
        return None;
    }
    // Every name is intercepted; each has a registry manifest (the intercepted names ⊆ registry).
    // Report it.
...
#[must_use]
pub fn help_for(line: &str, registry: &CommandRegistry, intercepted: &[&str]) -> Option<String> {
    let words = words(line)?;
    let words = crate::helpshim::skip_leading_sudo(&words);
    let (first, rest) = words.split_first()?;
    if !is_intercepted(first, intercepted) {
        return None;
    }
