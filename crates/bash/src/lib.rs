//! `bash` — clank's shell core: the Brush interpreter, the builtins, authorization, and the
//! [`Plugin`](plugin::Plugin) seam.
//!
...
// registration plumbing consumed from `tools`, `runtime` and `builtins` alike, not a builtin
// itself — kept alongside `registry` rather than under any one of those. Public because a plug-in
// crate registers its own hand-rolled `SimpleCommand`s through it.
pub mod helpshim;
pub mod logging;
pub mod manifest;
pub mod plugin;
...
pub mod runtime;
pub mod session;
// The internal tool implementations behind the registered builtins (coreutils, the text tools,
// `find`, `which`, …). Public because a plug-in crate renders its own listings through the same
// helpers the shell core does (`tools::coreutils::format_columns`, `tools::which`).
...
impl Transcript {
    ...
    /// Reachable from a consumer crate's tests through [`session::Session::set_context_cap`] under
