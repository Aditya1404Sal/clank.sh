...
fn read_supports_keyboard_enhancement_raw() -> io::Result<bool> {
    ...
    loop {
        match poll_internal(
            // clank patch: 2000ms -> 250ms. Keyboard-enhancement detection is the same class of
            // DSR round-trip as the cursor query (cursor/sys/unix.rs) and must not stall a slow
            // terminal; a responsive terminal answers well inside 250ms.
