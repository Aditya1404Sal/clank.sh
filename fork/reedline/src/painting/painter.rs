...
impl Painter {
    ...
    pub(crate) fn initialize_prompt_position(
        ...
        // clank patch: `cursor::position()` blocks on a Device-Status-Report (`\x1b[6n`) round-trip
        // and TIMES OUT when the terminal answers late — notably right after a long command like
        // `ask` dumps a burst of output. Upstream propagates that error with `?`, which aborts
        // read_line and forces the host into a degraded fallback (no hints/highlighting, a visible
        // stall). It is the only intolerant cursor query here; the rest already use `if let Ok(..)`.
        // Fall back to (col 0, bottom row): clank always ends command output in a newline, so the
        // cursor is at the start of a fresh line — column 0 is exactly right, and the bottom row is
        // where a new prompt belongs. This makes reedline usable on any terminal with slow DSR.
        let cursor = cursor::position().unwrap_or((0, self.terminal_size.1.saturating_sub(1)));
        let prompt_selector = select_prompt_row(suspended_state, cursor);
